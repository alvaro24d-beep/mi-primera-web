...
### The point cloud: a SECOND global canvas, on purpose

`GearPointsLazy` → `GearPointsCanvas` mounts its own `<Canvas>` at `z-index: -999` — between the wall (-1000) and the DOM. It holds a single `<points>` shaped like a spoked wheel and nothing else. It is deliberately *not* inside `SceneCanvas`: in there it inherited two limits that aren't its own — the wall's repaint cost (any mouse move had to invalidate a full-screen 300-line shader + Bloom just to move the points, so the interaction ran at the wall's frame rate) and the wall's `dpr` (1.25 desktop / **1 mobile**, low on purpose), which is what made the points look low-res on phones. Alone it runs at `dpr 2` and 60fps for a fraction of the cost, and the wall stops repainting on every mouse move — net less work than having them together.

**The source `.glb` is never loaded**: its surface was area-sampled offline into `public/gear-points.bin` (22.000 points, Int16-quantized + `normalized`, 129KB — vs. 7,6MB and 244k triangles). Regenerate with `node scripts/muestrear-glb.js <modelo.glb> public/gear-points.bin 22000`; the `.glb` is not in the repo. Density matters more than count: if you change the scale, re-sample — at 0.52 of the short side, 9.000 points were too sparse to read as a shape. Mouse repulsion runs in the vertex shader in *screen space* (so it stays correct while the wheel spins) and reads a 10-entry **trail** of past cursor positions that decays over ~1,6s, which is what makes points drift back instead of snapping. Mobile draws half the points via `setDrawRange` — any prefix of the array is still a uniform sample.
