...
**The source `.glb` is never loaded**: its surface was area-sampled offline into `public/gear-points.bin` (22.000 points, Int16-quantized + `normalized`, 129KB — vs. 7,6MB and 244k triangles). Regenerate with `node scripts/muestrear-glb.js <modelo.glb> public/gear-points.bin 22000`; the `.glb` is not in the repo. Only 10.000 of those points are actually drawn (`PUNTOS_DIBUJADOS`, half that on mobile) via `setDrawRange` — the sampling was random, so any prefix of the array is still a uniform sample.

**Count and point size are one decision, not two**, and getting it wrong is what produced three rounds of "it looks low-res" (V17.89→V17.91). With 22.000 points they have to be tiny to avoid empasting into each other, and below ~3px of diameter a disc has no pixels left to draw an interior *or* an edge — the whole figure then reads as noise, which is exactly the texture of a low-resolution image. Halve the count and each point can double in size at the same total coverage (~125.000 px², i.e. the same silhouette) while staying individually distinguishable. If you change `escala`, re-balance both together.

Two more things that fought sharpness, both fixed and both easy to reintroduce: the material must use `NormalBlending`, **not** additive (this is a 3D surface, so front face, back face and tangent edges all overlap on screen; additive saturates those overlaps to solid white masses), and the points must stay **below the composer's Bloom threshold** (`luminanceThreshold={0.6}`) — Bloom runs at `resolutionScale={0.5}`, so anything that crosses it gets a half-resolution blurred copy of itself composited on top. `uOpacity`·`vBrillo` peaks at 0.55 for that reason; raising `uOpacity` past ~0.50 brings the halo back.
