...
- `GearPoints` — a point cloud shaped like a spoked wheel, floating between the wall and the DOM. **The source `.glb` is never loaded**: its surface was area-sampled offline into `public/gear-points.bin` (9.000 points, Int16-quantized, 53KB — vs. 7,6MB and 244k triangles for the model). Regenerate it with `node scripts/muestrear-glb.js <modelo.glb> public/gear-points.bin 9000`; the `.glb` is not in the repo. Mouse repulsion happens in the vertex shader in *screen space* (so it stays correct while the wheel rolls), scroll spins it on its own Z axis, and mobile draws half the points via `setDrawRange` — any prefix of the array is still a uniform sample.
