...
import { useDwhStore } from "@/store/useDwhStore";
import { useReducedMotion } from "@/hooks/useReducedMotion";

const PERF_C = 2 * Math.PI * 26;

function FacetLabel() {
  const activeFacet = useDwhStore((s) => s.activeFacet);
  ...
  return (
    <div className="nxr-dwh-layers-panel nxr-glass-edge">
      <div className="nxr-glass-edge-content nxr-dwh-layers-inner">
        <span className="nxr-dwh-layer-num" style={{ color: facet.color }}>
          0{activeFacet + 1}
        </span>
        <div>
          <div className="nxr-dwh-layer-title">{facet.title}</div>
          <div className="nxr-dwh-layer-desc">{facet.desc}</div>
        </div>
      </div>
    </div>
  );
...
export default function DesarrolloWebHero() {
  ...
  useGSAP(
    () => {
      ...
      const setActiveFacet = useDwhStore.getState().setActiveFacet;
      ...
      // start (an empty window — a clear subject at rest), and only its
      // *content* builds up. The CSS default is the finished site, so the
      // reduced-motion path shows a built page; here we reset the content to
      // "before" and let the timeline construct it.
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          onUpdate: (self) => setActiveFacet(Math.min(FACETS.length - 1, Math.floor(self.progress * FACETS.length))),
        },
      });
      ...
      tl.to(q(".nxr-bw-phone"), { opacity: 1, xPercent: 0, rotateY: -20, duration: 0.8, ease: "power3.out" }, 3.1);
    ...
    <section key="animated" id="nxr-dwh-hero" className="nxr-dwh-hero" ref={sectionRef}>
      <div className="nxr-dwh-stage" ref={stageRef}>
        ...
        <div className="nxr-dwh-overlay">
          ...
          <FacetLabel />
          <div className="nxr-dwh-scrollhint">Scroll para construir ↓</div>
        </div>
      </div>
    </section>
  );
