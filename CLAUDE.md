...
**The source `.glb` is never loaded**: its surface was area-sampled offline into `public/gear-points.bin` (22.000 points, Int16-quantized + `normalized`, 129KB — vs. 7,6MB and 244k triangles). Regenerate with `node scripts/muestrear-glb.js <modelo.glb> public/gear-points.bin 22000`; the `.glb` is not in the repo. 13.000 of them are drawn on desktop, 2.800 on mobile (`PUNTOS_DIBUJADOS` / `PUNTOS_DIBUJADOS_MOVIL`) via `setDrawRange` — the sampling was random, so any prefix of the array is still a uniform sample.

**Count and point size are one decision, not two**, and getting it wrong is what produced several rounds of "it looks low-res" (V17.89→V17.93). What has to stay constant is the **px² of screen per point** (~38 currently) — *not* the point count, which is why "mobile draws half" was a bug: the figure covers 4.5× less area there (scale caps at 360 vs 700, and area goes with the radius squared), so half the points was more than double the density, and they empasted into each other. If you change `escala`, rebalance `uSize` and both counts together.

`uSize` 1.8 is the **hard floor**, and it has to be read in *framebuffer* pixels — where rasterization actually happens — not CSS ones: on desktop at `dpr` 1.25 that is a 1,58px sprite with a **1,19px disc**, barely over one pixel. Below this the disc stops covering a whole pixel, spreads across several at partial alpha, and every point becomes a translucent grey speck instead of a white dot — the figure reads as noise again, which is the "low-res" look that took several versions to remove. **To go smaller, raise the desktop `dpr` (currently 1.25); `uSize` has nothing left to give.** Note the limit is set by the *monitor*, not the phone — since V17.93 mobile runs at `dpr` 2, so the same CSS size gets a comfortable 1,9px disc there. It stays one shared value precisely so the phone spends its extra density on quality rather than shrinking the points to 0,8px CSS, where the cloud would vanish. At this size nothing may blur the point's surroundings again: no halo, no additive blending, no bloom (below). Those three used to compensate for each other at 4px; here none of them can come back.
