...
`uSize` 2.4 ≈ 1,7px on screen — and measured where it actually matters, in *framebuffer* pixels, that is a 2,0px sprite with a **1,5px disc on desktop**. This is the **floor**: 1,5px is the minimum at which a circle can have an interior *and* an edge at once, and below it the disc degenerates into a couple of grey pixels and the figure reads as noise. To go smaller you must first raise the canvas `dpr`, not lower `uSize` further. Note the limit is set by the *monitor*, not the phone — since V17.93 mobile runs at `dpr` 2 vs desktop's 1.25, so the same CSS size gets more real pixels there (2,5px disc). At this size nothing may blur the point's surroundings again: no halo, no additive blending, no bloom (below). Those three used to compensate for each other at 4px; here none of them can come back.
